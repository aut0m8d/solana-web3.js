...
const toBuffer = arr => {
  if (node_buffer.Buffer.isBuffer(arr)) {
    return arr;
  } else if (arr instanceof Uint8Array) {
    return node_buffer.Buffer.from(arr.buffer, arr.byteOffset, arr.byteLength);
  } else {
    return node_buffer.Buffer.from(arr);
  }
};
...
class Struct {
  ...
  encode() {
    return node_buffer.Buffer.from(borsh.serialize(SOLANA_SCHEMA, this));
  }
  static decode(data) {
...
class PublicKey extends Struct {
   ...
   */
  toBuffer() {
    const b = this._bn.toArrayLike(node_buffer.Buffer);
    if (b.length === PUBLIC_KEY_LENGTH) {
      return b;
    }
    const zeroPad = node_buffer.Buffer.alloc(32);
  ...
  static async createWithSeed(fromPublicKey, seed, programId) {
    const buffer = node_buffer.Buffer.concat([fromPublicKey.toBuffer(), node_buffer.Buffer.from(seed), programId.toBuffer()]);
  ...
  static createProgramAddressSync(seeds, programId) {
    let buffer = node_buffer.Buffer.alloc(0);
    seeds.forEach(function (seed) {
      if (seed.length > MAX_SEED_LENGTH) {
        throw new TypeError(`Max seed length exceeded`);
      }
      buffer = node_buffer.Buffer.concat([buffer, toBuffer(seed)]);
    });
    buffer = node_buffer.Buffer.concat([buffer, programId.toBuffer(), node_buffer.Buffer.from('ProgramDerivedAddress')]);
    const publicKeyBytes = sha256.sha256(buffer);
  ...
  static findProgramAddressSync(seeds, programId) {
    ...
    while (nonce != 0) {
      try {
        const seedsWithNonce = seeds.concat(node_buffer.Buffer.from([nonce]));
...
class Account {
   ...
   */
  get secretKey() {
    return node_buffer.Buffer.concat([this._secretKey, this._publicKey], 64);
  }
}
...
const rustString = (property = 'string') => {
  ...
  rslShim.encode = (str, b, offset) => {
    const data = {
      chars: node_buffer.Buffer.from(str, 'utf8')
    };
    return _encode(data, b, offset);
  };
  rslShim.alloc = str => {
    return BufferLayout__namespace.u32().span + BufferLayout__namespace.u32().span + node_buffer.Buffer.from(str, 'utf8').length;
  };
  return rslShim;
};
...
class Message {
  ...
  serialize() {
    ...
    const instructions = this.instructions.map(instruction => {
      ...
      return {
        programIdIndex,
        keyIndicesCount: node_buffer.Buffer.from(keyIndicesCount),
        keyIndices: accounts,
        dataLength: node_buffer.Buffer.from(dataCount),
        data
      };
    });
    let instructionCount = [];
    encodeLength(instructionCount, instructions.length);
    let instructionBuffer = node_buffer.Buffer.alloc(PACKET_DATA_SIZE);
    node_buffer.Buffer.from(instructionCount).copy(instructionBuffer);
    ...
    const transaction = {
      numRequiredSignatures: node_buffer.Buffer.from([this.header.numRequiredSignatures]),
      numReadonlySignedAccounts: node_buffer.Buffer.from([this.header.numReadonlySignedAccounts]),
      numReadonlyUnsignedAccounts: node_buffer.Buffer.from([this.header.numReadonlyUnsignedAccounts]),
      keyCount: node_buffer.Buffer.from(keyCount),
      keys: this.accountKeys.map(key => toBuffer(key.toBytes())),
      recentBlockhash: bs58__default.default.decode(this.recentBlockhash)
    };
    let signData = node_buffer.Buffer.alloc(2048);
  ...
  static from(buffer) {
    ...
    for (let i = 0; i < accountCount; i++) {
      ...
      accountKeys.push(new PublicKey(node_buffer.Buffer.from(account)));
    }
    ...
    for (let i = 0; i < instructionCount; i++) {
      ...
      const data = bs58__default.default.encode(node_buffer.Buffer.from(dataSlice));
      instructions.push({
        programIdIndex,
        accounts,
        data
    ...
    const messageArgs = {
      ...
      recentBlockhash: bs58__default.default.encode(node_buffer.Buffer.from(recentBlockhash)),
      accountKeys,
      instructions
    };
 ...
 */
const DEFAULT_SIGNATURE = node_buffer.Buffer.alloc(SIGNATURE_LENGTH_IN_BYTES).fill(0);
...
class TransactionInstruction {
  constructor(opts) {
     ...
     * Program input
     */
    this.data = node_buffer.Buffer.alloc(0);
...
class Transaction {
  ...
  _addSignature(pubkey, signature) {
    ...
    this.signatures[index].signature = node_buffer.Buffer.from(signature);
  }
  ...
  _serialize(signData) {
    ...
    const wireTransaction = node_buffer.Buffer.alloc(transactionLength);
    assert(signatures.length < 256);
    node_buffer.Buffer.from(signatureCount).copy(wireTransaction, 0);
    signatures.forEach(({
      signature
    }, index) => {
      if (signature !== null) {
        assert(signature.length === 64, `signature has invalid length`);
        node_buffer.Buffer.from(signature).copy(wireTransaction, signatureCount.length + index * 64);
      }
    });
  ...
  static from(buffer) {
    ...
    for (let i = 0; i < signatureCount; i++) {
      ...
      signatures.push(bs58__default.default.encode(node_buffer.Buffer.from(signature)));
    }
...
const bigInt = length => property => {
  ...
  bigIntLayout.decode = (buffer, offset) => {
    ...
    return bigintBuffer.toBigIntLE(node_buffer.Buffer.from(src));
  };
...
class Loader {
  ...
  static async load(connection, payer, program, programId, data) {
    ...
    while (array.length > 0) {
      ...
      const data = node_buffer.Buffer.alloc(chunkSize + 16);
      dataLayout.encode({
        instruction: 0,
        // Load instruction
        offset,
    ...
    // Finalize the account loaded with program data for execution
    {
      ...
      const data = node_buffer.Buffer.alloc(dataLayout.span);
      dataLayout.encode({
...
const BufferFromRawAccountData = superstruct.coerce(superstruct.instance(node_buffer.Buffer), RawAccountDataResult, value => node_buffer.Buffer.from(value[0], 'base64'));
...
const ParsedOrRawAccountData = superstruct.coerce(superstruct.union([superstruct.instance(node_buffer.Buffer), ParsedAccountDataResult]), superstruct.union([RawAccountDataResult, ParsedAccountDataResult]), value => {
...
class Connection {
  ...
  async simulateTransaction(transactionOrMessage, configOrSigners, includeAccounts) {
    if ('message' in transactionOrMessage) {
      ...
      const encodedTransaction = node_buffer.Buffer.from(wireTransaction).toString('base64');
...
class Ed25519Program {
  ...
  static createInstructionWithPublicKey(params) {
    ...
    const instructionData = node_buffer.Buffer.alloc(messageDataOffset + message.length);
...
class Secp256k1Program {
  ...
  static publicKeyToEthAddress(publicKey) {
    ...
    try {
      return node_buffer.Buffer.from(sha3.keccak_256(toBuffer(publicKey))).slice(-ETHEREUM_ADDRESS_BYTES);
    } catch (error) {
  ...
  static createInstructionWithEthAddress(params) {
    ...
    if (typeof rawAddress === 'string') {
      if (rawAddress.startsWith('0x')) {
        ethAddress = node_buffer.Buffer.from(rawAddress.substr(2), 'hex');
      } else {
        ethAddress = node_buffer.Buffer.from(rawAddress, 'hex');
      }
    } else {
      ethAddress = rawAddress;
    }
    ...
    const numSignatures = 1;
    const instructionData = node_buffer.Buffer.alloc(SECP256K1_INSTRUCTION_LAYOUT.span + message.length);
  ...
  static createInstructionWithPrivateKey(params) {
    ...
    try {
      ...
      const messageHash = node_buffer.Buffer.from(sha3.keccak_256(toBuffer(message)));
  ...
  static fromConfigData(buffer) {
    ...
    if (configKeys[0].publicKey.equals(VALIDATOR_INFO_KEY)) {
      if (configKeys[1].isSigner) {
        const rawInfo = rustString().decode(node_buffer.Buffer.from(byteArray));
